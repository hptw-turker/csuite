...
const TOK = await p.evaluate(async()=>{try{return await window.grecaptcha.execute(CAPTCHA.siteKey,{action:"talep"});}catch(e){return "HATA:"+e.message;}});
import("node:fs").then(fs=>fs.writeFileSync("/tmp/claude-0/-home-user-csuite/0c59bfa0-d975-5dba-9b7e-f2d86f4699ef/scratchpad/tok.json",JSON.stringify({token:TOK})));
await bekle(500);
console.log("TOKEN alindi, uzunluk", String(TOK).length);
